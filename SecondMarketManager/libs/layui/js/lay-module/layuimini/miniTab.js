...
layui.define(["element", "layer", "jquery"], function (exports) {
    ...
    var miniTab = {
        ...
        listenHash: function (options) {
            ...
            if (/noticeManage\/add\.html/i.test(tabId)) {
                sessionStorage.removeItem('layuiminimenu_' + tabId);
                location.hash = '/../noticeManage/index.html';
                return false;
            }
            if (tabId.indexOf('bannerManage/add.html') !== -1 && !/[?&]id=/.test(tabId)) {
                sessionStorage.removeItem('layuiminimenu_' + tabId);
                location.hash = '/../bannerManage/index.html';
                return false;
            }
