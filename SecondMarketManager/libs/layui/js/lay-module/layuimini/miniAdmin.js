    ...
    var miniAdmin = {
        ...
        filterMenuList: function (menuList) {
            function shouldHide(item) {
                var title = String(item && item.title || '').replace(/\s+/g, '');
                var href = String(item && item.href || '').toLowerCase();
                return title.indexOf('菜单管理') !== -1
                    || href.indexOf('menumanage') !== -1
                    || href.indexOf('usermanage/add.html') !== -1
                    || href.indexOf('noticemanage/add.html') !== -1
                    || title.indexOf('发布公告通知') !== -1
                    || href.indexOf('bannermanage/add.html') !== -1
                    || href.indexOf('page/menu.html') !== -1;
            }
            ...
            function walk(list) {
                ...
                $.each(list, function (_, item) {
                    var nextItem;
                    if (!item || shouldHide(item)) {
                        return;
                    }

                    nextItem = $.extend(true, {}, item);
                    if (Array.isArray(nextItem.child)) {
                        nextItem.child = walk(nextItem.child);
                        if (!nextItem.child.length && !nextItem.href) {
                            return;
                        }
                    }
        ...
        render: function (options) {
            ...
            $.getJSON(options.iniUrl, function (data) {
                if (data == null) {
                    ...
                    data.menuInfo = miniAdmin.filterMenuList(data.menuInfo);
        ...
        renderHome: function (data) {
            var homeSrc = data.href;
            if (homeSrc.indexOf('?') === -1) {
                homeSrc += '?_v=' + Date.now();
            } else {
                homeSrc += '&_v=' + Date.now();
            }
            ...
            $('#layuiminiHomeTabIframe').html('<iframe width="100%" height="100%" frameborder="no" border="0" marginwidth="0" marginheight="0"  src="' + homeSrc + '"></iframe>');
        },
